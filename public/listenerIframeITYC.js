...
if(readVal) drawThemeFrame = readVal;

readVal = window.localStorage.getItem('addOnGameSize');
if(readVal) gameSizeFrame = readVal;
...
window.addEventListener("load", () => {
  setTimeout(() => {  
    ...
    let spacer =  0;
    document.documentElement.setAttribute("data-theme", drawThemeFrame);
...
/*        try {
          const response = await chrome.runtime.sendMessage({type:"data", url, req: JSON.stringify(body), resp: text});
 ...
 //     chrome.runtime.sendMessage({type:"data", url, req: JSON.stringify(body), resp: text});
...
function manageAnswer(msg) {
    ...
    if(dashState == "notInstall") drawDashBoardDetected();
    if(msg.type=="update") {
    	manageGameInfos(msg);
      if(msg.rstTimer)
        chrono.Start();
...
{
  if(msg.type=="update" && msg.content == "") return;
  if(msg.theme)
  {
	  drawThemeFrame = msg.theme;
    window.localStorage.setItem('addOnTheme', msg.theme);
    document.documentElement.setAttribute("data-theme", drawThemeFrame);
  }
  if(msg.gameSize != undefined)
  {
    gameSizeFrame = msg.gameSize;
    window.localStorage.setItem('addOnGameSize', msg.gameSize);
    manageFullScreen2();
    sendParameter2Top(drawThemeFrame,gameSizeFrame);
  }
}
...
function manageGameInfos(msg) {
    ...
    if(!msg) return;
    if(msg.content!="")fillDashContainer(msg.content);
    if(msg.rid && msg.rid !="") {
...
function manageFullScreen2() {
  ...
  try {
    ...
    {// game is loaded
      ...
      {
        if(gameSizeFrame!=0 && window.parent != window)
        {
          adjustedSizeW = Math.ceil(maxScreenWidth*(gameSizeFrame==0?1:gameSizeFrame/100));
          adjustedSizeH = Math.ceil(maxScreenHeight*(gameSizeFrame==0?1:gameSizeFrame/100));
        }
    ...
    if(sendSize) {
      ...
      idC = document.getElementById("dashIntegRow");
      idC.setAttribute("data-theme", drawThemeFrame); 
