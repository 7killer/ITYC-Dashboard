...
setData: function setData(data) {
    ...
    if (this._windy) {
      // Met à jour les données dans Windy…
      this._windy.setData(data);
      // …et relance un start SANS effacer le canvas (voir _clearAndRestart patché plus bas)
      this._startWindy();
    }
  ...
  _clearAndRestart: function _clearAndRestart() {
    // ⚠ On ne nettoie plus brutalement le canvas ici pour éviter le "flash" / clignotement.
    // On se contente de relancer le calcul du champ et l'animation.
...
var Windy = function Windy(params) {
  ...
  var animate = function animate(bounds) {
    ...
    // On seed les particules une seule fois, sur le champ courant
    for (var i = 0; i < particleCount; i++) {
      particles.push(windy.field.randomize({
    ...
    function evolve() {
      ...
      particles.forEach(function (particle) {
        ...
        var x = particle.x;
        var y = particle.y;
        var v = windy.field(x, y); // vector at current position
  ...
  var start = function start(bounds, width, height, extent) {
    ...
    // On ne stoppe plus l'animation à chaque changement de données :
    // on recalcule juste un nouveau champ, et on ne lance animate(bounds)
    // qu'une seule fois, au tout premier appel.
    buildGrid(gridData, function (grid) {
      // interpolateField
      interpolateField(grid, buildBounds(bounds, width, height), mapBounds, function (bounds, field) {
        if (!windy.field) {
          // Premier démarrage : champ + particules + boucle d'anim
          windy.field = field;
          animate(bounds);
        } else {
          // MàJ du champ seulement : les particules continuent,
          // mais utilisent le nouveau champ de vent.
          windy.field = field;
        }
      });
    });
  ...
  var stop = function stop() {
    if (windy.field) {
      // Libère les ressources associées au champ courant
      windy.field.release();
    }

    // Marque le champ comme "absent" : le prochain start()
    // relancera animate() et recréera les particules.
    windy.field = null;
