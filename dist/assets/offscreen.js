...
import { f as formatGPRMC, a as formatIIMWV, b as formatIIVWR, d as formatIIHDT, e as formatRPM, n as nmeaChecksum, g as formatAIVDM_AIS_msg1, h as formatAIVDM_AIS_msg5 } from "./nmeaUtils-8a8ec4be.js";
import "./utils-068774e3.js";
const NMEA_DELAY = 1e3;
...
function applySnapshot(snapshot = {}) {
  ...
  if ("raceId" in snapshot)
    state.raceId = snapshot.raceId ?? null;
  if ("legNum" in snapshot)
    state.legNum = snapshot.legNum ?? null;
  if ("currentUserId" in snapshot)
    state.currentUserId = snapshot.currentUserId ?? null;
  if ("playerInfo" in snapshot)
    state.playerInfo = snapshot.playerInfo ?? null;
  if ("fleetInfo" in snapshot)
    state.fleetInfo = Array.isArray(snapshot.fleetInfo) ? snapshot.fleetInfo : [];
}
...
function canSendNmea() {
  return state.requested && state.running && state.raceId && state.legNum && state.playerInfo;
}
function canSendAis() {
  return state.requested && state.running && state.raceId && state.legNum && Array.isArray(state.fleetInfo) && state.fleetInfo.length > 0 && state.interval <= NMEA_DELAY_MAX;
}
async function sendNmeaBurst() {
  try {
    ...
    if (state.retry++ > 2) {
      state.interval *= 2;
      if (state.interval > NMEA_DELAY_MAX)
