...
export const  Windy = function( params ){
  ...
  var createWindBuilder = function(uComp, vComp) {
   ...
   var createBuilder = function(data) {
      // CAS 1 : format "GRIB-like" (array de records U/V)
      if (Array.isArray(data)) {
          ...
          data.forEach(function(record) {
              ...
              switch (record.header.parameterCategory + "," + record.header.parameterNumber) {
                  case "2,2": uComp = record; break; // UGRD
                  case "2,3": vComp = record; break; // VGRD
                  default:
                      scalar = record;
              }
          });
      ...
      // CAS 2 : format "leaflet-windy / windpack"
      //   {
      //     header: { lo1, la1, dx, dy, nx, ny, refTime, forecastTime? },
      //     data: [ uArray, vArray ]
      //   }
      if (data && data.header && Array.isArray(data.data) && data.data.length >= 2) {
          ...
          var commonHeader = {
              ...
              nx: header.nx,
              ny: header.ny,
              refTime: header.refTime || "197001010000",   // pour compat avec ton WindyDataProxy.interpolateData
